...
public class UserController {
    ...
    private static final int SHORT_PAGE_SIZE = 4;
    ...
    public String viewUserExchangesHistory(Model model) {
        ...
        if (user != null) {
            model.addAttribute("walletAmount", user.getWalletAmount());
            model.addAttribute("userMargin", user.getUserMargin());
            model.addAttribute("userEmail", currentUser);
        }
        if (orders != null) {
    ...
    public String viewUserNotifications(Model model) {
        String currentUser = SecurityContextHolder.getContext().getAuthentication().getName();
        ExchangerUser user = userService.findByEmail(currentUser);
        List<ExchangeOrder> orders = exchangeService.getAllExchangeOrders(user);
        if (user != null) {
    ...
    public String completeOrder(@Valid @ModelAttribute("note") ExchangeOrder order, Model model) {
        ...
        String network = coinService.getCoinByCoinSymbol(order.getGivenCoin()).getNetwork();
        exchangeService.makeAnExchange(order, network);
    ...
    @GetMapping("/review/part")
    public String showPartReviews(Model model, @RequestParam("page") Optional<Integer> page, @RequestParam("size") Optional<Integer> size) {
        int currentPage = page.orElse(FIRST_PAGE);
        int pageSize = size.orElse(SHORT_PAGE_SIZE);
